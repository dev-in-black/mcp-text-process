...
class SSEHandler:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                yield await self._send_event(
                    "chunk", {"index": i, "chunk": chunk, "length": len(chunk)}
                )
        ...
        try:
                ...
                while True:
                    ...
                    matches.append(
                        type(
                            "Match",
                            (),
                            {"start": lambda: pos, "end": lambda: pos + len(find)},
                        )()
                    )
                    start = pos + 1
            ...
            yield await self._send_event(
                "matches_found",
                {
                    "count": len(matches),
                    "matches": [
                        {"start": m.start(), "end": m.end()} for m in matches[:10]
                    ],  # Limit to first 10
                },
            )
    ...
    async def stream_fuzzy_delete(
        ...
        try:
            yield await self._send_progress(
                0, 1, f"Starting fuzzy deletion with {algorithm} algorithm..."
            )
            ...
            for i, word in enumerate(words):
                similarity = self.text_processor._calculate_similarity(
                    word, target, algorithm
                )
                ...
                if i % 100 == 0:
                    yield await self._send_progress(
                        i, len(words), f"Analyzing word {i+1}/{len(words)}"
                    )
                    await asyncio.sleep(0.01)

            yield await self._send_event(
                "matches_found",
                {"count": len(matches), "matches": matches[:10]},  # Limit to first 10
            )
            ...
            yield await self._send_progress(
                1, 1, f"Deleted {len(matches)} fuzzy matches"
            )
            yield await self._send_result(result)
            yield await self._send_completion()
    ...
    async def stream_batch_process(
        self, texts: List[str], operation: str, parameters: Dict[str, Any]
    ) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            yield await self._send_progress(
                0, total_texts, f"Starting batch {operation} on {total_texts} texts..."
            )
            ...
            for i, text in enumerate(texts):
                try:
                    yield await self._send_progress(
                        i, total_texts, f"Processing text {i+1}/{total_texts}"
                    )

                    # Process individual text
                    if operation == "split":
                        result = await self.text_processor.split_text(
                            text, **parameters
                        )
                    ...
                    yield await self._send_event(
                        "batch_error", {"index": i, "error": str(e)}
                    )
                    results.append(None)

            yield await self._send_progress(
                total_texts, total_texts, "Batch processing completed"
            )
