...
class TestTextProcessor:
    ...
    async def test_find_replace_case_insensitive(self):
        ...
        text = "Hello world, hello universe"
        result = await self.processor.find_replace(text, "hello", "hi", case_sensitive=False)
    ...
    async def test_find_replace_regex(self):
        ...
        result = await self.processor.find_replace(text, r'\d{4}-\d{2}-\d{2}', "YYYY-MM-DD", regex=True)
    ...
    async def test_fuzzy_delete_exact(self):
        ...
        result = await self.processor.fuzzy_delete(text, "apple", similarity_threshold=1.0)
    ...
    async def test_fuzzy_delete_similar(self):
        ...
        result = await self.processor.fuzzy_delete(text, "apple", similarity_threshold=0.7)
    ...
    async def test_search_partial(self):
        ...
        results = await self.processor.search(text, "quick brown", search_type="partial")
    ...
    def test_similarity_calculation(self):
        ...
        # Test exact match
        similarity = self.processor._calculate_similarity("apple", "apple", "levenshtein")
        assert similarity == 1.0
        
        # Test different strings
        similarity = self.processor._calculate_similarity("apple", "banana", "levenshtein")
        assert similarity < 1.0
